...
<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>"><img class="img-thumbnail" src="<?php $values = get_post_custom_values("picture"); echo $values[0]; ?>" alt="<?php the_title(); ?>" /></a>
<?php } /* else {
     // 设置特色图像
...
function post_to_sina_weibo($post_ID) {
    ...
    // 将 abc 替换成你的新浪微博登陆名
    $username = "798105509@qq.com";
    // 将 123 替换成你的新浪微博密码
    $password = "1999111519991115";
    
    $get_post_info = get_post($post_ID);
    
    if ( $get_post_info->post_status == 'publish' && $_POST['original_post_status'] != 'publish' ) {
        $request = new WP_Http;
        $status = strip_tags( $_POST['post_title'] ) . ' ' . urlencode( get_permalink($post_ID) ) . ' ' . strip_tags( $_POST['excerpt'] ) ;
        $api_url = 'http://api.t.sina.com.cn/statuses/update.json';
        $body = array( 'status' => $status, 'source'=>'52612119');
